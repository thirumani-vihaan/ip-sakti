...
4. **Embeddings for offline tests:** deterministic fake embeddings (seeded) so retrieval tests are reproducible without network. Offline can optionally use a real local `sentence-transformers` model (opt-in via `USE_LOCAL_EMBEDDINGS=1`); the live path uses Gemini `gemini-embedding-001`.
